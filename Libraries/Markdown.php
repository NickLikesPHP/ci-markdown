 ...
 * @link        https://github.com/NickLikesPHP/ci-markdown
 *
 * @version     1.7.0
 * @version     PHP Markdown Lib 2.0.0
 */
class Markdown {
    /**
     ...
     *
     * @param array $override_params Override settings configured in Config/Markdown.php.
     */
    public function __construct(array $override_params = array()){
        
        $config_file = config('Markdown');
        
        if(is_null($config_file)){
            log_message('critical', 'Markup Config file not found. Defaulting to built-in config.');
        }
        
        $this->load_settings($config_file, $override_params);
    ...
    /**
     * Loads settings from the specified $config array.
     *
     * @param object $markdown_config the markdown config object
     * @param array $override_params Override settings configured in Config/Markdown.php.
     */
    protected function load_settings($markdown_config, $override_params){
        $this->tab_width = $override_params['tab_width'] ?? $markdown_config->tab_width ?? $this->tab_width;
        $this->no_markup = $override_params['no_markup'] ?? $markdown_config->no_markup ?? $this->no_markup;
        $this->no_entities = $override_params['no_entities'] ?? $markdown_config->no_entities ?? $this->no_entities;
        $this->hard_wrap = $override_params['hard_wrap'] ?? $markdown_config->hard_wrap ?? $this->hard_wrap;
        $this->predef_urls = $override_params['predef_urls'] ?? $markdown_config->predef_urls ?? $this->predef_urls;
        $this->predef_abbr = $override_params['predef_abbr'] ?? $markdown_config->predef_abbr ?? $this->predef_abbr;
        $this->predef_titles = $override_params['predef_titles'] ?? $markdown_config->predef_titles ?? $this->predef_titles;
        $this->fn_id_prefix = $override_params['fn_id_prefix'] ?? $markdown_config->fn_id_prefix ?? $this->fn_id_prefix;
        $this->fn_link_title = $override_params['fn_link_title'] ?? $markdown_config->fn_link_title ?? $this->fn_link_title;
        $this->fn_backlink_title = $override_params['fn_backlink_title'] ?? $markdown_config->fn_backlink_title ?? $this->fn_backlink_title;
        $this->fn_link_class = $override_params['fn_link_class'] ?? $markdown_config->fn_link_class ?? $this->fn_link_class;
        $this->fn_backlink_class = $override_params['fn_backlink_class'] ?? $markdown_config->fn_backlink_class ?? $this->fn_backlink_class;
        $this->fn_backlink_html = $override_params['fn_backlink_html'] ?? $markdown_config->fn_backlink_html ?? $this->fn_backlink_html;
        $this->table_align_class_tmpl = $override_params['table_align_class_tmpl'] ?? $markdown_config->table_align_class_tmpl ?? $this->table_align_class_tmpl;
        $this->code_class_prefix = $override_params['code_class_prefix'] ?? $markdown_config->code_class_prefix ?? $this->code_class_prefix;
        $this->code_attr_on_pre = $override_params['code_attr_on_pre'] ?? $markdown_config->code_attr_on_pre ?? $this->code_attr_on_pre;
        $this->enhanced_ordered_list = $override_params['enhanced_ordered_list'] ?? $markdown_config->enhanced_ordered_list ?? $this->enhanced_ordered_list;
        $this->empty_element_suffix = $override_params['empty_element_suffix'] ?? $markdown_config->empty_element_suffix ?? $this->empty_element_suffix;
        $this->hashtag_protection = $override_params['hashtag_protection'] ?? $markdown_config->hashtag_protection ?? $this->hashtag_protection;
        $this->omit_footnotes = $override_params['omit_footnotes'] ?? $markdown_config->omit_footnotes ?? $this->omit_footnotes;
    }
